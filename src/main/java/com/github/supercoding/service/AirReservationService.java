...
import com.github.supercoding.respository.airplane.airlineTicket.AirlineTicketAndFlightInfo;
...
import com.github.supercoding.respository.airplane.airlineTicket.AirlineTicketRepository;
...
import com.github.supercoding.respository.airplane.passenger.PassengerRepository;
import com.github.supercoding.respository.airplane.reservation.FlightPriceAndCharge;
import com.github.supercoding.respository.airplane.reservation.ReservationEntity;
import com.github.supercoding.respository.airplane.reservation.ReservationJpaRepository;
import com.github.supercoding.respository.airplane.reservation.ReservationRepository;
import com.github.supercoding.respository.airplane.users.UserEntity;
import com.github.supercoding.respository.airplane.users.UserJpaRepository;
...
public class AirReservationService {
	...
	public List<Ticket> findUserFavoritePlaceTickets(Integer userId, String ticketType) {
		...
		List<AirlineTicketEntity> airlineTickets
	...
	public ReservationResult makeReservation(ReservationRequest reservationRequest) {
		...
		AirlineTicketEntity airlineTicketEntity = airlineTicketJpaRepository.findById(airlineTicketId).orElseThrow(() -> new NotFoundException("airLineTicket 찾을 수 없습니다."));

		// 1. Passenger I
		PassengerEntity passenger = passengerJpaRepository.findPassengerEntityByUserEntityUserId(userId).orElseThrow(() -> new NotFoundException("요청하신 userId " + userId + "에 해당하는 Passenger를 찾을 수 없습니다."));


		// 2. price 등의 정보 불러오기
		List<FlightEntity> flightList = airlineTicketEntity.getFlightList();
		...
		// 3. reservation 생성
		ReservationEntity reservation = new ReservationEntity(passenger, airlineTicketEntity);
		try {
		...
		List<Integer> prices = flightList.stream().map(FlightEntity::getFlightPrice).map(Double::intValue).collect(Collectors.toList());
		List<Integer> charges = flightList.stream().map(FlightEntity::getCharge).map(Double::intValue).collect(Collectors.toList());
		Integer tax = airlineTicketEntity.getTax().intValue();
		Integer totalPrice = airlineTicketEntity.getTotalPrice().intValue();
